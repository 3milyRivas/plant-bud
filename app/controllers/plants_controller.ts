...
export default class PlantsController {
    private API_KEY = 'rmsPszQkVVhi9Wsz69OcEQPcGXQXsZZiUKaiN3f0XFX1h57hKV'
    ...
    public async scan({ request, response }: HttpContext) {
        ...
        const plant_info = this.getPlantInfo(top, result)
        const plant_education = this.buildEducation(
            species,
            confidence,
            health,
            plant_info
        ...
        return (suggestions || []).slice(0, 6).map((s: any) => {
            ...
            return {
                ...
                image:
                    ...
                    s?.plant_details?.url ||
                    null,
                google_search: `https://www.google.com/search?q=${encodeURIComponent(
                    `${name} plant`
                )}`,
            }
        })
    ...
    private getHealth(result: any) {
        ...
        const healthyProbability =
            healthAssessment?.healthy_probability || 0
    ...
    private buildEducation(
        ...
        return {
            description:
                ...
                `${species} was identified with ${confidence}% confidence.`,

            taxonomy_summary: {
                kingdom:
                    plant_info?.taxonomy?.kingdom || 'Unknown',
                family:
                    plant_info?.taxonomy?.family || 'Unknown',
                genus:
                    plant_info?.taxonomy?.genus || 'Unknown',
                order:
                    plant_info?.taxonomy?.order || 'Unknown',
            },

            facts: [
                `Identification confidence: ${confidence}%`,
                `Health status: ${health.status}`,
                `Recommended watering: ${plant_info?.watering}`,
                `Recommended sunlight: ${plant_info?.sunlight}`,
            ],
